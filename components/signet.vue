...
<template>
    ...
    <div class="flex flex-col lg:flex-row w-full">
        <div
            class="py-6 grid grid-cols-3 place-items-center w-full gap-3 bg-gradient-to-b from-dark-pink to-dark-violet">
        ...
        <div class="flex flex-col w-full text-white">
            <div class="flex w-full px-4 py-3 bg-gradient-to-b from-dark-violet to-dark-blue">
                    ...
                    <PopoverGroup v-else class="flex flex-col w-full relative">
                        <Popover>
                            ...
                            <Transition name="slide-fade">
                                <PopoverPanel class="absolute top-0 left-0 rounded z-10 h-fit max-h-64 overflow-y-auto bg-dark-pink border border-white px-2 py-1">
                                    {{ selectedSignet.note }}
                                    <div v-show="selectedSignet.danger">
                                        <span class="font-semibold underline">Warning!</span> <br /> {{ selectedSignet.danger }}
                                    </div>
                                </PopoverPanel>
                            </Transition>
                        </Popover>
        ...
        <div class="bg-dark-blue text-white h-fit max-h-72 overflow-y-auto mt-6">
            <table class="w-full border border-collapse">
                <tr v-for="(item, index) in selectedSignet.lists" :key="index">
                    <td class="border border-white py-3">
                        <button @click="descriptionModal = item.description; isOpen = true" class="flex items-center space-x-2">
