...
<template>
    <div class="flex space-x-2 w-40">
        <button @click="first.length > 1 ? swapSupport('first') : ''" class="relative w-full" @mouseenter="$emit('is-hover', true, fetchDescription(first[firstSection]))" @mouseleave="$emit('is-hover', false)">
            <img class="absolute border border-dark-blue bg-light-yellow w-16 h-16 z-20 left-0 top-0"
                :src="`/supports/${first[firstSection]}.webp`" :alt="first[firstSection]" :key="firstSection">
            <img class="absolute border border-dark-blue bg-light-yellow w-16 h-16 z-0 left-2 top-2" v-show="first.length > 1"
                :src="`/supports/${first[nextFirstSection]}.webp`" :alt="first[nextFirstSection]"
                :key="nextFirstSection">
        </button>
        <button @click="second.length > 1 ? swapSupport('second') : ''" class="relative w-full" @mouseenter="$emit('is-hover', true, fetchDescription(second[secondSection]))" @mouseleave="$emit('is-hover', false)">
            <img class="absolute border border-dark-blue bg-light-yellow w-16 h-16 z-20 left-0 top-0"
                :src="`/supports/${second[secondSection]}.webp`" :alt="second[secondSection]" :key="secondSection">
            <img class="absolute border border-dark-blue bg-light-yellow w-16 h-16 z-0 left-2 top-2" v-show="second.length > 1"
